...
type EventHandler interface {
	...
	OnReceive(conn *conn) ([]byte, error)

	OnShutdown(conn *conn) error

	OnClose(conn *conn, err error) error
}

type conn struct {
	fd             int             // file descriptor
	ctx            context.Context // user-defined context
	...
	//pollAttachment netpoll.PollAttachment // connection attachment for selector
...
func (c *conn) release() {
	...
	if addr, ok := c.localAddr.(*net.TCPAddr); ok && len(c.loop.listeners) == 0 && len(addr.Zone) > 0 {
		bsPool.Put(StringToBytes(addr.Zone))
	}
...
loop:
	if sent, err = unix.Write(c.fd, data); err != nil {
		...
		if err == unix.EAGAIN {
			...
			if !isET {
				err = c.loop.selector.ModReadWrite(c.ctx, c.fd, false)
			}
			return
		}
...
func (c *conn) writev(bs [][]byte) (n int, err error) {
	...
	var sent int
loop:
	if sent, err = socket.Writev(c.fd, bs); err != nil {
		// A temporary error occurs, append the data to outbound buffer,
		// writing it back to the remote in the next round for LT mode.
		if err == unix.EAGAIN {
...
//	return c.loop.selector.Trigger(queue.LowPriority, func(_ interface{}) (err error) {
//		err = c.loop.wake(c)
//		if callback != nil {
//			_ = callback(c, err)
//		}
...
type Connections struct {
	//connections []*conn       // loop connections storage
...
func (c *Connections) GetConnMap() map[int]*conn {
	return c.ConnMap
}

func (c *Connections) AddConn(conn *conn) error {
	if conn == nil {
		return errors.WithStack(errors.New("connection is nil"))
	}
...
func (c *Connections) GetConnByFd(fd int) *conn {
	return c.ConnMap[fd]
}

func (c *Connections) GetCount() int {
	return len(c.ConnMap)
}

func (c *Connections) DelConn(delConn *conn) {
	if delConn != nil {
		delete(c.ConnMap, delConn.Fd())
	}
}
